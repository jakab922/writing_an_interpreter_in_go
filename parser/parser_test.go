...
func testIdentifier(t *testing.T, exp ast.Expression, value string) bool {
	...
	if !ok {
		t.Errorf("exp not *ast.Identifier got %T instead", exp)
		return false
	}
...
func testLiteralExpression(
	...
	t.Errorf("type of exp is not handled. The type was %T", exp)
	return false
}
...
func testInfixExpression(t *testing.T, exp ast.Expression, left interface{},
	...
	if !ok {
		t.Errorf("exp is not ast.OperatorExpression. Got %T(%s) instead", exp, exp)
		return false
	}
	...
	if opExp.Operator != operator {
		t.Errorf("exp.Operator is not %q. Got %q instead",
...
func TestBooleanExpression(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if !ok {
			t.Fatalf("Was expecting an ast.Boolean but got %T", stmt.Expression)
		}

		if boolean.Value != tt.expected {
			t.Errorf("Expected boolean value %t, but got %t",
...
func testBooleanLiteral(t *testing.T, exp ast.Expression, value bool) bool {
	...
	if !ok {
		t.Errorf("exp not *ast.Boolean. Got %T instead", exp)
		return false
	}

	if bo.Value != value {
		t.Errorf("bo.Value not %t, got %t instead", value, bo.Value)
		return false
	}

	if bo.TokenLiteral() != fmt.Sprintf("%t", value) {
		t.Errorf("bo.TokenLiteral not %t, got %s instead",
...
func TestIfExpression(t *testing.T) {
	...
	if len(program.Statements) != 1 {
		t.Fatalf("Expected 1 statement got %d instead",
			len(program.Statements))
	}
	...
	if !ok {
		t.Fatalf("stmt.Expression is not ast.IfExpression. Got %T instead",
			stmt.Expression)
	}
	...
	if !ok {
		t.Fatalf("Statements[0] is not ast.ExpressionStatement. Got %T instead",
...
func TestIfElseExpression(t *testing.T) {
	...
	consequence, ok := exp.Consequence.Statements[0].(*ast.ExpressionStatement)
	if !ok {
		t.Fatalf("exp.Consequence.Statements[0] is not ast.ExpressionStatement. Got %T instead",
			exp.Consequence.Statements[0])
	}
	...
	if !ok {
		t.Fatalf("exp.Alternative.Statements[0] is not ast.ExpressionStatement. Got %T instead",
...
func TestFunctionLiteralParsing(t *testing.T) {
	...
	if len(program.Statements) != 1 {
		t.Fatalf("program.Body does not contain %d statements. Intead there are %d statements",
			1, len(program.Statements))
	}
	...
	if !ok {
		t.Fatalf("stmt.Expression is not ast.FunctionLiteral. Got %T instead",
			stmt.Expression)
	}

	if len(function.Parameters) != 2 {
		t.Fatalf("function literal should have 2 parameters. Got %d instead",
	...
	if len(function.Body.Statements) != 1 {
		t.Fatalf("body.Statements has not 1 statements. Got %d instead",
			len(function.Body.Statements))
	}

	estmt, ok := function.Body.Statements[0].(*ast.ExpressionStatement)
	if !ok {
		t.Fatalf("function body stmt is not ast.ExpressionStatement. Got %T instead.",
			function.Body.Statements[0])
	}

	testInfixExpression(t, estmt.Expression, "x", "+", "y")
}
...
func TestCallExpressionParsing(t *testing.T) {
	...
	if len(program.Statements) != 1 {
		t.Fatalf("There should be only one program statements. There are %d instead",
			len(program.Statements))
	}

	stmt, ok := program.Statements[0].(*ast.ExpressionStatement)
	if !ok {
		t.Fatalf("stmt is not an ast.ExpressionStatement. Instead it is a %T.",
			program.Statements[0])
	}

	exp, ok := stmt.Expression.(*ast.CallExpression)
	if !ok {
		t.Fatalf("stmt.Expression is not an ast.CallExpression. Got %T instead",
			stmt.Expression)
	}

	if !testIdentifier(t, exp.Function, "add") {
		t.Fatalf("The identifier of the function should be add")
	}

	if len(exp.Arguments) != 3 {
		t.Fatalf("There should be 3 arguments. Got %d instead.",
			len(exp.Arguments))
	}
